      ...
      <Card>
        ...
        <div className="space-y-3">
          {[
            ...
            <div
              key={item.query}
              className="flex items-center justify-between gap-3 px-4 py-3 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors cursor-pointer"
            >
              <div className="min-w-0">
                <p className="font-medium text-gray-900 truncate">{item.query}</p>
                <p className="text-sm text-gray-500">{item.date}</p>
              </div>
              <span className="px-3 py-1 bg-brand-50 text-brand-700 text-sm font-semibold rounded-full shrink-0">
                {item.score}점
              </span>
            </div>
          ))}
